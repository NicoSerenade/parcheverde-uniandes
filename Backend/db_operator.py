...
def register_user(student_code, username, email, password, career=None, interests=None):
    ...
    if conn is not None:
        try:
            ...
            VALUES (?, ?, ?, ?, ?, ?)
            ''', (student_code, username, email, hashed_password, career, interests)) 
            conn.commit()
...
def authenticate_user(student_code, password):
    ...
    if conn is not None:
        try:
            ...
            cursor.execute('''
            SELECT user_id, student_code, username, email, password, career, interests, points, creation_date
            FROM users
            WHERE student_code = ?
...
def update_user_profile(user_id, student_code=None, username=None, email=None, password=None, career=None, interests=None):
    """
...
def get_user_by_id(user_id):
    ...
    if conn is not None:
        try:
            cursor = conn.cursor()
            cursor.execute('''
            SELECT user_id, student_code, username, email, password, career, interests, points
            FROM users
            WHERE user_id = ?
            ''', (user_id,))
            user = cursor.fetchone()
